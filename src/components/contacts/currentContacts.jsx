...
const CurrentContacts = () => {
    ...
    useEffect(()=>{
        if(value.length>0){
            const results = userContacts.filter(user=> user.username.toLowerCase().includes(value.toLocaleLowerCase() || user._id.toLocaleLowerCase().includes(value.toLocaleLowerCase())))
    ...
    return ( 
            ...
            <ContactList select={null} selected={null} contacts={searchResult} shouldSelect={false}/>
            </div>
