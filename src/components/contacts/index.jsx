...
const ContactsComp = () => {
    ...
    const handleCloseDropDown = (e)=>{
        if (dropDownRef.current && dropDownToggleRef.current && !dropDownRef.current.contains(e.target) && !dropDownToggleRef.current.contains(e.target)) {
    ...
    return ( 
        <div className='w-full h-full p-4'>
            <div className='w-full flex flex-row items-center justify-between' >
            {
                    ...
                    <BackArrow/>
                     <div>Contacts</div>
                    <img ref={dropDownToggleRef} onClick={()=>setDropDownShowing(!dropDownShowing)} className={`w-[30px] h-[30px]  transition-[5000] ${dropDownShowing?"":`rotate-90`} `} src="../chevron.svg" alt="" />
                    </div>
            ...
            <ul ref={dropDownRef} className={` ${!dropDownShowing && `hidden` } bg-midGray w-[200px] rounded-lg border-mainGray  border z-30 absolute  text-white `}>
