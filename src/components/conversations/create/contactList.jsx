...
const ContactList = ({selected, select, contacts, shouldSelect=true}) => {
    ...
    useEffect(()=>{
        ...
        for (let letter of alphabets){
            ...
            contacts.forEach(contact => {if(contact._id[0].toUpperCase() == letter) listOfContacts.push(contact)})
       ...
       <div className='w-full lg:grid grid-cols-3 flex flex-col  mt-4 gap-4'>
          {
            ...
            :
            alphabet.listOfContacts.map(contact => 
                <ContactWithoutSelect key={contact._id} username={contact.username} image={contact.profilePic?contact.profilePic.url:"../userIcon.svg"} _id={contact._id}/>
